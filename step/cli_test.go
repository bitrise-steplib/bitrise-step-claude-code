...
func TestBuildClaudeArgs_AgentsJSON(t *testing.T) {
	input := Input{
		Prompt: "hello",
		APIKey: "sk-test",
	}
	agentsJSON := `{"code-review":{"description":"Reviews code","prompt":"Review this"}}`

	args, err := buildClaudeArgs(input, "", agentsJSON)
	require.NoError(t, err)

	assertContainsSequence(t, args, "--agents", agentsJSON)
}

func TestBuildClaudeArgs_NoAgents(t *testing.T) {
	input := Input{
		Prompt: "hello",
		APIKey: "sk-test",
	}
